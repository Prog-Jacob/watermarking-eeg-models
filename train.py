...
match architecture:
  case "CCNN":
    ...
    dataset = DEAPDataset(io_path=f"{working_dir}/dataset",
...
def train():
  ...
  for i, (train_dataset, test_dataset) in enumerate(cv.split(dataset)):
    ...
    match architecture:
      ...
      case _: raise ValueError(f"Invalid architecture: {architecture}")
    ...
    if not skip_training:
      ...
      if experiment == "nowatermark":
        val_dataset = test_dataset
        trigger_set = train_dataset
      else:
        verifier = Verifier.CORRECT
        if experiment.startswith("newwatermark"): verifier = Verifier.NEW
        val_dataset = TriggerSet(test_dataset, architecture, include_train_set=True, verifier=verifier)
        trigger_set = TriggerSet(train_dataset, architecture, include_train_set=True, verifier=verifier)

      val_loader = DataLoader(val_dataset, shuffle=True, batch_size=batch_size, num_workers=4)
      train_loader = DataLoader(trigger_set, shuffle=True, batch_size=batch_size, num_workers=4)
    ...
    ckpt_file = next((os.path.join(save_path, f) for f in os.listdir(save_path) if f.endswith('.ckpt')), None)
    model = load_model(model, ckpt_file)
    model.eval()

    for eval_dimension in evals:
      if eval_dimension.ends_with("watermark"):
        verifier = Verifier[eval_dimension.split("_")[0].upper()]
        null_set = TriggerSet(test_dataset, architecture, seed=42, do_true_embedding=False, verifier=verifier)
        true_set = TriggerSet(test_dataset, architecture, seed=42, do_null_embedding=False, verifier=verifier)
        
        null_set_loader = DataLoader(null_set, batch_size=batch_size)
        true_set_loader = DataLoader(true_set, batch_size=batch_size)
        
        results[fold][eval_dimension] = {
        ...
        results[fold][eval_dimension] = trainer.test(test_loader, enable_model_summary=True)
...
def calculate_accuracy(results_path):
  samples = 0
  test_accuracy = 0
  null_set_accuracy = 0
  true_set_accuracy = 0
  ...
  for subjects in results.values():
      for scores in subjects.values():
          ...
          null_set_accuracy += scores['null_set_test_accuracy']
          true_set_accuracy += scores['true_set_test_accuracy']

  test_accuracy /= samples
  null_set_accuracy /= samples
  true_set_accuracy /= samples

  print(f'Test Accuracy is {test_accuracy}')
  print(f'Null Set Accuracy is {null_set_accuracy}')
  print(f'True Set Accuracy is {true_set_accuracy}')

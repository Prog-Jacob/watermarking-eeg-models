...
def get_watermark(architecture, verifier_string, private_key):
    ...
    match architecture:
        ...
        case _: raise ValueError("Invalid architecture!")
...
def verify(verifier_string, signature, public_key):
    try:
        rsa.verify(verifier_string.encode('UTF-8'), b64decode(signature.encode('UTF-8')), public_key)
...
class TriggerSet(Dataset):
    def __init__(
        ...
        filter, wm_label = get_watermark(architecture, verifier, private_key)
        
        if len(train_set) < size: trigger_set = train_set
        else: trigger_set = [train_set[random.randint(0, len(train_set) - 1)] for _ in range(size)]
        
        null_embedding = [apply_null_embedding(s, filter, t) for (s, t) in trigger_set if do_null_embedding]
        true_embedding = [apply_true_embedding(s, filter, wm_label) for (s, _) in trigger_set if do_true_embedding]

        if not include_train_set: trigger_set = []
        else: trigger_set = [sample for sample in train_set]
